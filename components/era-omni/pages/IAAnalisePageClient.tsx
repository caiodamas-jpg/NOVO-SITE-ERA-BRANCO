...
import NPSAnalysisAnimation from "../NPSAnalysisAnimation"
import { getOmniPageBySlug, getOmniRelatedPages } from "@/data/era-omni-pages"

const pageData = getOmniPageBySlug("ia-analise")!
const related = getOmniRelatedPages(pageData.relatedSlugs)

export default function IAAnalisePageClient() {
  ...
  return (
    <>
        ...
        <div className="max-w-5xl mx-auto">
          <nav aria-label="Breadcrumb" className="mb-6">
            <ol className="flex items-center gap-1.5 text-xs text-gray-500">
              ...
              <li><Link href="/era-omni" className="hover:text-gray-900 transition-colors">Mensageria</Link></li>
              <li><ChevronRight className="w-3 h-3" /></li>
      ...
      <section className="py-16 px-6 border-t border-gray-200 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {related.map((p) => (
              <Link key={p.slug} href={`/era-omni/${p.slug}`} className="block p-5 rounded-xl border border-gray-200/50 bg-white hover:border-gray-300 transition-colors group">
      ...
      <BlogAndSlideshow tags={["ia-analise", "omnichannel", "atendimento"]} categoryProduct="era-omni" />
